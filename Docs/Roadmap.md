...
The engine must hold a living Commonwealth, not a village. Design
locked 2026-08-11 — Docs/Design/Scale.md: memory cap default 0,
Update stays the raw primitive, the core single-threaded by
construction, the core proves the cost shape and the adapter owns
the in-game wall clock, soak runs two cadences.

[✓] Stone 13 — Tick budgets and profiling: TickReport — per-pass
    counts (minds swept, events faded, pairs drifted) and per-pass
    wall time; nullptr (the default) measures nothing, existing
    callers untouched. The documented cost model: per tick
    ~Σ over minds of (O(events) + O(relationships)) + O(population),
    bounded only by the memory cap. Proven by the Scale suite.
[✓] Stone 14 — Iteration efficiency: sim.memory.cap (default 0 =
    unbounded, unchanged behavior; the world tunes it) evicts the
    lowest-weight event on insert, bounding the hot path — and
    FixedStep, the timing-independent tick: real frame deltas in,
    whole fixed steps out, same seed + same steps = same world at
    any frame rate. Update(delta) stays the primitive; FixedStep is
    the opt-in helper. Proven by the Scale suite (the carry
    mechanics + two worlds at different frame rates, bit-identical).
[✓] Stone 15 — Soak tests: a decade at day-steps (300 settlers,
    3650 days) and an hour at the real cadence (30 settlers, 36,000
    × 0.1s steps) — no NaN, no entity creep, memory bounded, the
    world provably moved. The finding is the point; so far: stable.
[✓] Stone 16 — Save/load at population scale: 5000 minds
    round-trip byte-exactly; 207 bytes per mind in the co-save
    (needs + three memory events + one relationship) — the
    documented per-entity cost.
[✓] Stone 17 — Determinism at scale: two worlds, same seed, 1000
    minds — flattened snapshots byte-for-byte identical.

Proof: a settlement of hundreds ticks inside a frame budget; a year
of sim time passes without drift — the FixedStep accumulator makes
"without drift" mean "same seed, any frame rate". The game-thread
decision (0.4.0) is revisited here and kept: the core stays
single-threaded and deterministic; threading the tick off the game
thread is the adapter's execution choice (F4SE's TaskInterface is
already waiting on its side). The adapter's in-game verification is
the remaining gate — set sim.memory.cap once it measures, adopt
FixedStep for the tick loop, pass a TickReport when it wants
per-frame costs; the co-save schema is unchanged.
